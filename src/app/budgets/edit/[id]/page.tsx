...
export default function Page({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        ...
        async function fetchEditingBudget() {
            try {
                const editingBudget = await fetchBudgetById(params.id)
                
                if(editingBudget) {
                    console.log(editingBudget)
                    // setNewBudget({ budgetTypeId: editingBudget.budgetTypeId, budgetTotal: String(editingBudget.budgetTotal) });
                } else redirect('/budgets');
            } catch (error) {
